...
void shoot_init(void)
{
    ...
    //电机指针
    shoot_control.shoot_motor_measure = get_pluck_motor_measure_point();
    //初始化PID
    PID_init(&shoot_control.trigger_motor_pid, PID_USUAL, Trigger_speed_pid, TRIGGER_READY_PID_MAX_OUT, TRIGGER_READY_PID_MAX_IOUT);
    //更新数据
...
static void shoot_set_mode(void)
{
    ...
    if (switch_is_up(current_s))
    {
        ...
        {
            // 切换 STOP <-> READY_FRIC
            shoot_control.shoot_mode = (shoot_control.shoot_mode == SHOOT_STOP) ? 
                                        SHOOT_READY_FRIC : SHOOT_STOP;
        }
    }
    /* 下拨触发射击 */
    ...
    if (shoot_control.shoot_mode == SHOOT_READY_FRIC && 
        shoot_control.fric1_ramp.out >= FRIC_UP * 0.95f && 
        shoot_control.fric2_ramp.out >= FRIC_UP * 0.95f)
    {
